...
ファイル [ のglobal.js](/dotnet/core/tools/global-json) に依存して特定の .NET Core SDK バージョンを対象とする場合は、 `version` インストールされている .net 5.0 SDK バージョンにプロパティを更新します。 以下に例を示します。
...
## <a name="changes-to-blazor-app-routing-logic-in-501-and-further-5x-releases-up-to-60"></a>Blazor5.0.1 でのアプリルーティングロジックの変更と、6.0 までのさらに5.x リリース
...
## <a name="update-blazor-webassembly-and-blazor-server-projects"></a>更新プログラム Blazor WebAssembly と Blazor Server プロジェクト
...
1. `Program.Main`( `Program.cs` ) で、ハッシュを追加して、要素への参照を `<app>` CSS セレクターに変更し `#` ます。
...
1. `Program.Main`( `Program.cs` ) で、既定の一時的な `HttpClient` 登録をスコープに変更します (存在する場合)。
...
1. `Program.Main`ホストさ `Program.cs` れている **`Client`** ソリューションのアプリの () () Blazor :
...
### <a name="standalone-blazor-webassembly-app-with-microsoft-accounts"></a>Blazor WebAssemblyMicrosoft アカウントを使用したスタンドアロンアプリ
...
### <a name="standalone-blazor-webassembly-app-with-azure-active-directory-aad"></a>Blazor WebAssemblyAzure Active Directory を使用したスタンドアロンアプリ (AAD)
...
### <a name="standalone-blazor-webassembly-app-with-azure-active-directory-aad-b2c"></a>Blazor WebAssemblyAzure Active Directory (AAD) B2C を使用したスタンドアロンアプリ
...
### <a name="hosted-blazor-webassembly-app-with-azure-active-directory-aad-or-aad-b2c"></a>Blazor WebAssemblyAzure Active Directory (AAD) または AAD B2C を使用したホストされたアプリ
...
### <a name="update-the-server-project-of-a-hosted-blazor-solution"></a>ホストされているソリューションのサーバープロジェクトを更新する Blazor
...
## <a name="update-a-blazor-progressive-web-application-pwa"></a>Blazorプログレッシブ Web アプリケーション (PWA) を更新する
...
## <a name="update-razor-class-libraries-rcls"></a>Razorクラスライブラリの更新 (RCLs)
...
## <a name="model-binding-changes-in-aspnet-core-mvc-and-razor-pages"></a>MVC とページ ASP.NET Core のモデルバインドの変更 Razor
